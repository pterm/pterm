...
func main() {
	...
	pterm.Println("The following table has no rgb (supported by every terminal), no axis data and a custom legend.\n")

	table := pterm.DefaultHeatmap.WithData(data).WithBoxed(false).WithAxisData(headerData).WithLegend(false).WithLegendTag("custom").WithLegendOnlyColoredCells()
	table.Render()

	pterm.Println("The following table has rgb (not supported by every terminal), axis data and a custom legend.\n")
	table2 := pterm.DefaultHeatmap.WithData(data).WithBoxed(false).WithAxisData(headerData).WithRGB().WithLegendTag("custom").WithLegendOnlyColoredCells()
	table2.Render()
}
