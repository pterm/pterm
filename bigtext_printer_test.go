...
func TestBigTextPrinterNilPrint(_ *testing.T) {
...
func TestDefaultLettersMaxHeight(t *testing.T) {
	...
	for s, l := range chars {
		h := strings.Count(l, "\n")
		assert.True(t, h <= maxHeight, fmt.Sprintf("'%s' is too high", s))
	}
}
